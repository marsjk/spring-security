...
public class OAuth2LoginSecurityConfig {
	...
	private LogoutSuccessHandler oidcLogoutSuccessHandler() {
		...
		// Optional opaque value that can be used to maintain state between the logout
		// request and the callback to the endpoint specified by the above setPostLogoutRedirectUri
		oidcLogoutSuccessHandler.setState("some_opaque_value");
